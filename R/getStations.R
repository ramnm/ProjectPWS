...
#' @param state US two character state code, a city may also be supplied. If no city is supplied, all cities returned for that state will be queried (may take a lot of time).
#' @param country Name of a (non-US) country. If no city is supplied, all cities returned for that country will be queried (may take a lot of time).
#' @param city Name of the requested city.
#' @param radius Optional radius in miles to limit results to. Must be a numeric value greater than 0 but less than 25. If querying by state or country, the behavior here may not be intuitive.
...
getStations <- function(latlong = NA, zip = NA, state = NA, country = NA, city = NA, radius = NA) {
  ...
  # This helper function assumes that the returned page contains the nearby_weather_stations node
  ...
  # If the query is for a whole state or country, then we need to query all cities in that area
  if (!is.na(state) || !is.na(country) && is.na(city)) {
    ...
    cityNames <- sapply(XML::getNodeSet(areaRoot, areaPath, areaNamespace), XML::xmlValue)

    cityTotal <- length(cityNames)
    cityCount <- 1
    print(sprintf("There are %d cities to query. This may take a while.", cityTotal))
    cityTables <- lapply(cityNames, function(cityName) {
      print(sprintf("Processing city '%s' (%d of %d)...", cityName, cityCount, cityTotal))
      cityCount <<- cityCount + 1
      getAndParseStationData(paste0(pwstationObj$queryArg,
                                    "/",
                                    cityName))
    })
