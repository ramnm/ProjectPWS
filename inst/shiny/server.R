...
#
      ...
      a <- paste0(substr(a,6,7),"/",substr(a,9,10),"/",substr(a,1,4))
      b <- as.character(isolate(input$endDate))
      b <- paste0(substr(b,6,7),"/",substr(b,9,10),"/",substr(b,1,4))
      c <- switch(isolate(input$wParm),
                  "Temperature" = "tempi",
                  "Humidity" = "hum",
                  "Wind Speed" = "windS",
                  "Pressure" = "press")
      wRange <<- TRUE
      ProjectPWS::loadWeatherData(pwStations = obj,
                                  startDate = a,
                                  endDate = b,
                                  weatherVars = c)
    }
      ...
      timeRange <- sapply(seq_along(dateRange),function(x){
        if(x == 1){
          if (length(dateRange) == 1) {
            ...
            temp <<- paste(dateRange[x],timesRef[time1:nrow(timesRef)])
          }
        } else {
